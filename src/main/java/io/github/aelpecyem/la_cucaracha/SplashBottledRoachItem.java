	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
		...
		if (!world.isClient) {
			...
			entity.setVelocity(user, user.getPitch(), user.getYaw(), -20.0F, 0.5F, 1.0F);
					//FABRIC DISCORD:entity.refreshPositionAndAngles(user.getX(), user.getY(), user.getZ(), user.getYaw(), 0.0f);
					//OLD:entity.setProperties(user, user.getPitch(), user.getYaw(), -20.0F, 0.5F, 1.0F);
